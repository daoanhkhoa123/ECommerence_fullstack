...
# def create_order_item_product(customer_id: int, request, uow: SqlAlchemyUnitOfWork) -> OrderItem:
#     with uow:
...
#         vp = uow.products.get(request.vendor_product_id)
#         if not vp:
...
# def find_order_item_product(customer_id: int, order_item_id: int, uow: SqlAlchemyUnitOfWork) -> OrderItem:
#     with uow:
...
# def find_all_order_items_by_order(customer_id: int, order_id: int, uow: SqlAlchemyUnitOfWork) -> List[OrderItem]:
#     with uow:
...
# def decrease_stock_by_order_item(order_item: OrderItem, uow: SqlAlchemyUnitOfWork):
#     with uow:
...
# def delete_order_item(customer_id: int, order_item_id: int, uow: SqlAlchemyUnitOfWork):
#     with uow:
#         order_item = uow.order_items.get(order_item_id)
#         if not order_item:
#             raise HTTPException(status_code=404, detail=f"OrderItem {order_item_id} not found")
